...
@contextmanager
def get_db():
    """Helper to get a database connection that auto-closes."""
...
def setup_db():
    """Run schema.sql to create tables. Also migrate old databases."""
    try:
    ...
    # MIGRATION: remove 'type' column from old notifications table if it exists
    try:
        conn = sqlite3.connect(DATABASE_FILE)
        cursor = conn.cursor()
...
def request_collab(post_id):
    """Send a collab request on a post."""
    if "user_id" not in session:
...
def decline_request(request_id):
    """Decline a pending collab request."""
    if "user_id" not in session:
        return jsonify({"error": "Please login first!"}), 401
